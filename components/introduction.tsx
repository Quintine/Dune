...
import { CharityLesson, RevivalLesson } from './introduction-resources';
import { StormLesson, SpiceBlowLesson } from './introduction-opening';
import {
...
export function Introduction() {
  ...
  return <main className={styles.page}>
    ...
    {hydrated ? <IntroductionLessons key="v6" /> : <output>Restoring your lesson…</output>}
  </main>;
}
...
function IntroductionLessons() {
  ...
  return <>
      ...
      <section className={styles.lesson} aria-labelledby="lesson-title">
        ...
        {state.step === 0 && <>
          <p>Each faction has its own powers. Practice Basic bidding as Fremen, then shipping and battles as Atreides with ordinary forces. Advanced combat and expansion powers come later.</p>
        ...
        {state.step === 1 && <StormLesson state={state} update={update} />}
        {state.step === 2 && <SpiceBlowLesson state={state} update={update} />}
        {state.step === 3 && <AllianceLesson state={state} update={update} />}
        {state.step === 4 && <CharityLesson state={state} update={update} />}
        {state.step === 5 && <BiddingLesson state={state} update={update} />}
        {state.step === 6 && <RevivalLesson state={state} update={update} />}
        {state.step === 7 && <>
        ...
        {state.step === 8 && <MovementLesson state={state} update={update} />}
        {state.step === 9 && <>
        ...
        {state.step === 10 && <TraitorLesson state={state} update={update} />}
        {state.step === 11 && <>
