...
export function IxSubstitution({
  ...
  const available = Object.entries(me.forces)
    .filter(([key]) => splitLocation(key).territory === d.territory)
    .map(
      ([key, count]) => [key, count - (me.elites?.forces[key] ?? 0)] as const,
    )
  ...
  return (
    <>
      <p>
        ...
        battle. Retained cyborgs return to their casualty sectors. Opponents may
        cancel this substitution with Karama.
      </p>
      <h3>Suboids to sacrifice</h3>
      {available.map(([key, max]) => (
        <label key={key}>
          Sector {splitLocation(key).sector} · {max} suboids available
          <Input
            type="number"
            min={0}
            max={max}
      ...
      {Object.entries(d.losses).map(([key, max]) => (
        <label key={key}>
          Sector {splitLocation(key).sector} · {max} cyborgs lost
          <Input
            type="number"
            min={0}
            max={max}
      ...
      <Button
        disabled={busy || !total(sources) || total(sources) !== total(recover)}
