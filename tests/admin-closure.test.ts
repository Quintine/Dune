...
const change = (
  ...
  expectedRevision,
  closed,
  reason: 'Dedicated QA room closure',
});
function seed(store: Store, code = 'CLOSURES') {
...
async function fixture(role: AdminRole = 'operator') {
  ...
  const read = () => readAdminClosure(store.database, identity, room.code, now);
  const apply = (input = change(version()), at = now) =>
    applyAdminClosure(store.database, identity, room.code, input, at);
  return {
    ...store,
    ...room,
    account,
...
void test('closure migration is additive and preserves all existing rooms, credentials and audit records', () => {
  ...
  try {
    ...
    store.sqlite.exec(
      readFileSync(
        new URL('../drizzle/0013_admin_room_closure.sql', import.meta.url),
        'utf8',
      ),
    );
    assert.deepEqual(
    ...
    assert.deepEqual(rows(store, 'room_closures'), []);
    assert.deepEqual(rows(store, 'admin_room_closures'), []);
    assert.throws(
      () =>
        store.sqlite
          .prepare(
            'INSERT INTO room_closures(room_code,closed,revision,updated_at) VALUES(?,1,0,?)',
          )
          .run(room.code, now),
      /CHECK/,
    );
...
void test('closure inputs require exact bounded fields and both safe version counters', () => {
  const valid = change();
  assert.equal(validAdminClosureInput(valid), true);
  for (const invalid of [
    ...
    Object.assign(Object.create({ closed: true }), {
      operationId: valid.operationId,
      expectedVersion: 0,
      expectedRevision: 0,
      reason: 'Reason',
    ...
    assert.equal(validAdminClosureInput(invalid), false);
});
...
void test('live roles can read only operational defaults without backfilling or disclosing room and seat contents', async () => {
  for (const role of ['owner', 'operator', 'viewer'] as const) {
    ...
    try {
      ...
      assert.deepEqual(rows(f, 'room_closures'), []);
      if (role === 'viewer') await assert.rejects(f.apply(), status(403));
      else assert.equal((await f.apply()).room.closed, true);
      await assert.rejects(
        readAdminClosure(f.database, f.identity, f.code + '\n', now),
        status(400),
      );
      await assert.rejects(
        readAdminClosure(f.database, f.identity, 'ABSENTAA', now),
        status(404),
      );
      await assert.rejects(
        readAdminClosure(
...
void test('close and reopen preserve saved JSON, every dependent record, revoked access and unrelated rooms', async () => {
  ...
  try {
    ...
    assert.equal(rows(f, 'room_closures').length, 1);
    const audit = rows(f, 'admin_room_closures');
...
void test('closure and reopening tolerate unreadable saves without parsing diagnostics or repairing JSON', async () => {
  for (const state of [
...
void test('reopening refreshes existing AI pacing only when unpaused and preserves pause and joining flags', async () => {
  for (const paused of [false, true])
    for (const controller of ['bot', 'autopilot', 'human'] as const) {
      ...
      try {
        ...
        assert.equal(f.row().state, state);
        const reopend = await f.apply(
          change(f.version(), 1, false),
          now + 3000,
        );
        assert.equal(reopend.room.paused, paused);
        assert.equal(reopend.room.joinLocked, true);
...
void test('exact receipts bind actor, room and normalized intent and report current state after later transitions', async () => {
  ...
  try {
    ...
    await assert.rejects(
      applyAdminClosure(f.database, identity, f.code, input, now),
      status(409),
    );
    seed(f, 'KEEPSAFE');
    await assert.rejects(
      applyAdminClosure(f.database, f.identity, 'KEEPSAFE', input, now),
      status(409),
    );
    assert.deepEqual(f.row(), before);
    assert.equal(rows(f, 'admin_room_closures').length, 2);
  } finally {
    f.sqlite.close();
  }
});

void test('new no-op and stale requests reject without writes; both room version and closure revision fence races', async () => {
  const f = await fixture();
  try {
    const before = f.row();
    ...
    assert.deepEqual(f.row(), before);
    assert.deepEqual(rows(f, 'admin_room_closures'), []);
    ...
    assert.equal(rows(f, 'admin_room_closures').length, 1);
  } finally {
    f.sqlite.close();
  }
});
  ...
  for (const operation of ['read', 'write', 'replay'] as const)
    for (const revoke of [
      ...
      try {
        ...
        const before = f.row(),
          audit = rows(f, 'admin_room_closures');
        const gate = pauseAdminBatch(f);
        ...
        assert.deepEqual(f.row(), before);
        assert.deepEqual(rows(f, 'admin_room_closures'), audit);
      } finally {
        f.sqlite.close();
      }
    }
...
void test('audit, game fence and closure metadata roll back together at every failed persistence stage', async () => {
  for (const table of ['admin_room_closures', 'rooms', 'room_closures']) {
    const f = await fixture();
    try {
      const before = f.row(),
        secure = secureRows(f);
      f.sqlite.exec(
        `CREATE TRIGGER qa_failure BEFORE ${table === 'rooms' ? 'UPDATE' : 'INSERT'} ON ${table} BEGIN SELECT RAISE(ABORT,'QA closure failure'); END;`,
      );
      await assert.rejects(f.apply(), /QA closure failure/);
      assert.deepEqual(f.row(), before);
      assert.deepEqual(secureRows(f), secure);
      assert.deepEqual(rows(f, 'admin_room_closures'), []);
      assert.deepEqual(rows(f, 'room_closures'), []);
...
void test('closed games and exact receipts survive database reopen without restoring historical credentials', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'dune-admin-closure-'));
  ...
  try {
    ...
    const input = change();
    const closed = await applyAdminClosure(
      store.database,
      identity,
      room.code,
      input,
    ...
    assert.deepEqual(
      await applyAdminClosure(store.database, identity, room.code, input, now),
      { ...closed, replayed: true },
    );
    await applyAdminClosure(
      store.database,
      identity,
      room.code,
      change(1, 1, false),
    ...
    const audit = rows(store, 'admin_room_closures');
    store.sqlite.prepare('DELETE FROM rooms WHERE code=?').run(room.code);
    assert.deepEqual(rows(store, 'admin_room_closures'), audit);
  } finally {
    store.sqlite.close();
    rmSync(dir, { recursive: true, force: true });
  }
});
void test('closed rooms keep public admin reads and old receipts while new lobby and pause mutations reject', async () => {
  const f = await fixture();
  try {
    const lobbyInput = {
      operationId: randomUUID(),
...
void test('prepared admin lobby and pause writes lose their transaction race to closure', async () => {
...
void test('removal retains closure and restoring a closed room preserves AI deadlines until a separate reopen', async () => {
  const f = await fixture();
  try {
    const game = JSON.parse(String(f.row().state));
    game.status = 'playing'; game.players[0].autopilot = 'Hard'; game.botNextActionAt = 222; game.botsPending = true;
    f.sqlite.prepare('UPDATE rooms SET state=? WHERE code=?').run(JSON.stringify(game), f.code);
    const closing = change();
    await f.apply(closing);
    const state = f.row().state, secure = secureRows(f);
    const removal = { operationId: randomUUID(), expectedVersion: f.version(), expectedRevision: 0, removed: true, reason: 'QA removal while closed' };
    await applyAdminRemoval(f.database, f.identity, f.code, removal, now);
    assert.equal((await readAdminRemoval(f.database, f.identity, f.code, now)).closed, true);
    const current = await readAdminClosure(f.database, f.identity, f.code, now);
    assert.equal(current.removed, true); assert.equal(current.closed, true);
    await assert.rejects(f.apply(change(f.version(), 1, false)), status(409));
    assert.equal((await f.apply(closing)).replayed, true);
    assert.equal(f.row().state, state);
    await applyAdminRemoval(f.database, f.identity, f.code, { ...removal, operationId: randomUUID(), expectedVersion: f.version(), expectedRevision: 1, removed: false }, now + 50000);
    assert.equal(f.row().state, state);
    assert.equal((await readAdminClosure(f.database, f.identity, f.code, now)).closed, true);
    await applyAdminClosure(f.database, f.identity, f.code, change(f.version(), 1, false), now + 50000);
    assert.equal(JSON.parse(String(f.row().state)).botNextActionAt, now + 51500);
    assert.deepEqual(secureRows(f), secure);
