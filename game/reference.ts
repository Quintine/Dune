...
const phaseDetails = [
  ...
  [
    'The game collects spice automatically when this phase begins.',
...
export const RULE_TOPICS: RuleTopic[] = [
  ...
  ),
  {
    ...
    coverage: 'Partial',
    summary:
      'End-of-Revival placement, eight random entry effects and direct Ecaz Duke acquisition are supported in development fixtures, with public token guides and private inspection history.',
    steps: [
    ...
    checklist: [
      {
        ...
        status: 'Partial',
        detail:
          'Inventory, placement and eight entry effects plus their Bene Gesserit copies and direct Ecaz Duke acquisition are integrated. Storm/explosion token returns are integrated. Competing arrival ordering and remaining effects remain unfinished.',
      },
      {
        area: 'Player controls',
        status: 'Partial',
        detail:
          'Placement and entry controls show valid destinations, beneficiaries, copies and card choices with blocked reasons. Relocation controls select physical and elite forces by source sector, concealed markers and eligible advisor flips. Independent Guild shipment controls select typed reserve counts, destination sector or zero; BG accompaniment controls show source-specific blocked reasons. Public token guides and recipient-only historical card/identity inspectors remain available without acknowledgement.',
      },
      {
        area: 'AI',
        status: 'Partial',
        detail:
          'All four profiles handle placement, supported trigger/beneficiary choices, copied effects, legal CHOAM/Ixian card selection, Fremen relocation, Guild reserve shipment and its BG accompaniment from their own projections. Unsupported effects are declined; remaining effect policies are unfinished.',
      },
      {
        area: 'Documentation',
        status: 'Partial',
        detail:
          'Placement, supported entry decisions, private historical inspections, inventory lifecycle and all eleven effect descriptions are available. Complete timing and interaction coverage remains open.',
      },
      {
        area: 'Verification',
        status: 'Partial',
        detail:
          'Focused tests cover custody, placement payment, entry continuation, private snapshots, effect choices, destruction and all four bot profiles. Persisted tests cover concurrent triggers, private snapshot restoration, and stale-event rejection; browser checks verified entry and enlarged private cards. Guild-focused tests cover physical quotes, historical continuations, cancellation, resume provenance, actual engine journeys and all four AI profiles. Guild multiplayer and browser verification are pending at this documentation checkpoint. Complete expansion games and competing arrivals remain.',
        evidence: [
      ...
      steps:
        f.id === 'ecaz'
          ? [
              ...
              'Eight entry effects and their Bene Gesserit copies, plus direct Ecaz Duke acquisition, have focused support. Storm/explosion token returns, ordinary territorial co-occupation and the three jointly occupied stronghold victory are also supported. Tleilaxu revival, combined Occupy battles, shared desert collection, the Fremen endgame exception and the complete Duke revival and custody interactions remain unfinished. Consult the feature checklist for the exact tested boundary.',
            ]
          : f.id === 'moritani'
            ? [
  ...
  ),
  {
    ...
    steps: [
      ...
      'The current implementation supports Moritani acquisition, direct Ecaz Ambassador self-acquisition and one-battle use. Ecaz alliance/loan alternatives, reacquisition while already controlled, revival and captured-leader interactions remain unfinished. This shared component does not enable complete Ecaz or Moritani games. Inspection does not commit a battle plan.',
    ],
    related: [
      'faction-ecaz',
      'faction-moritani',
