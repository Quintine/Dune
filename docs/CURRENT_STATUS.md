...
[Bureaucrat payments](BUREAUCRAT_PAYMENTS.md) connect
...
samples finish with zero rejections; only the staged pair uses this effect. A staged
browser exercise verifies a Fedaykin return, three ordinary returns and refresh.
Final broad checks, preservation counts and Git delivery are recorded in the
checkpoint commit and private report before handoff.

The purchase alternative still needs a private post-win payment choice. The earlier
question asks whether to allow a uniform winner payment step, including otherwise
