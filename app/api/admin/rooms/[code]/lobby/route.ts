...
import { configureAdminLobby, readAdminLobby } from '@/db/admin-lobby-configuration';
import { validAdminLobbyInput } from '@/lib/admin-lobby-configuration';
...
export async function GET(request: Request) {
  try {
    ...
    return adminResponse(await readAdminLobby(env.DB, identity, code(request)));
...
export async function POST(request: Request) {
  try {
    ...
    if (request.headers.get('X-Dune-Admin-Id') !== identity.id)
      throw new AdminError('The signed-in administrator changed. Reload administration before continuing.', 401);
    if (!validAdminLobbyInput(body)) throw new AdminError('Choose a lobby change, revision and operational reason.', 400);
    return adminResponse(await configureAdminLobby(env.DB, identity, code(request), body));
  } catch (error) { return adminFailure(error); }
}
