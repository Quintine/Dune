...
import { validAdminArchiveInput } from '@/lib/admin-archive';
...
export async function GET(request: Request) {
  try {
    ...
    return adminResponse(await readAdminArchive(env.DB, identity, code(request)));
...
export async function POST(request: Request) {
  try {
    ...
    if (request.headers.get('X-Dune-Admin-Id') !== identity.id)
      throw new AdminError('The signed-in administrator changed. Reload administration before continuing.', 401);
    if (!validAdminArchiveInput(input)) throw new AdminError('Choose archive or unarchive, current versions and an operational reason.', 400);
    const room = code(request);
    const result = await applyAdminArchive(env.DB, identity, room, input);
