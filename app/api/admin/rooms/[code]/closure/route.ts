...
import { validAdminClosureInput } from '@/lib/admin-closure';
...
export async function GET(request: Request) {
  try {
    ...
    return adminResponse(await readAdminClosure(env.DB, identity, code(request)));
...
export async function POST(request: Request) {
  try {
    ...
    if (request.headers.get('X-Dune-Admin-Id') !== identity.id)
      throw new AdminError('The signed-in administrator changed. Reload administration before continuing.', 401);
    if (!validAdminClosureInput(input)) throw new AdminError('Choose closure or reopening, current versions and an operational reason.', 400);
    const room = code(request);
    const result = await applyAdminClosure(env.DB, identity, room, input);
    if (!result.replayed && !result.room.closed && !result.room.paused) waitUntil(resumeRoom(room));
