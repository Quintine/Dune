...
type Filters = { q: string; status: string; rules: string; sort: string };
const initialFilters: Filters = { q: '', status: 'all', rules: 'all', sort: 'recent' };
  ...
  return <main className="admin-shell">
    <header className="admin-header">
      ...
      {account && <div className="admin-session"><span>{account.name} <span className="admin-tag">{account.role}</span></span>
        <Button variant="outline" disabled={busy} onClick={() => void sessionAction('logout')}><LogOut aria-hidden="true" /> Sign out</Button></div>}
    </header>
    ...
    {restoring ? <output>Checking administrator access…</output> : !account ?
      <section className="admin-login" aria-labelledby="admin-sign-in">
        ...
        <form onSubmit={event => { event.preventDefault(); void sessionAction('login'); }}>
          ...
          <Input id="admin-key" type="password" autoComplete="current-password" autoCapitalize="none" spellCheck={false}
            maxLength={130} value={key} onChange={event => setKey(event.target.value)} required disabled={busy} />
          <Button type="submit" disabled={busy || !key.trim()}>{busy ? 'Signing in…' : 'Sign in'}</Button>
        </form>
      ...
      <section aria-labelledby="admin-rooms-title">
        <div className="admin-section-heading"><div><h2 id="admin-rooms-title">Rooms</h2><p className="admin-secondary">Public room details. Cards, sealed choices and private discussion stay hidden.</p></div>
          <Button variant="outline" disabled={busy} onClick={() => void load(applied, directory?.page ?? 1)}><RefreshCw aria-hidden="true" /> Refresh</Button></div>
        <form className="admin-filters" onSubmit={event => { event.preventDefault(); void load(filters); }}>
          <label className="admin-search" htmlFor="admin-search">Room code or player name<Input id="admin-search" value={filters.q} maxLength={80} disabled={busy} onChange={e => setFilters({ ...filters, q: e.target.value })} /></label>
          <label>Status<select disabled={busy} value={filters.status} onChange={e => setFilters({ ...filters, status: e.target.value })}>
          ...
          <label>Rules<select disabled={busy} value={filters.rules} onChange={e => setFilters({ ...filters, rules: e.target.value })}>
            <option value="all">All rules</option><option value="basic">Basic</option><option value="advanced">Advanced</option>
          </select></label>
          <label>Sort<select disabled={busy} value={filters.sort} onChange={e => setFilters({ ...filters, sort: e.target.value })}>
          ...
          {!directory ? <output>{busy ? 'Loading rooms…' : 'Use Refresh to load rooms.'}</output> : <>
            ...
            {directory.rooms.length === 0 ? <p>No rooms on this page. Change the filters or return to the first page.</p> :
              <Table className="admin-room-table"><TableHeader><TableRow>
                ...
                <TableCell><time dateTime={new Date(room.updatedAt).toISOString()}>{new Date(room.updatedAt).toLocaleString()}</time><p className="admin-secondary">Version {room.version}</p></TableCell>
              </TableRow>)}</TableBody></Table>}
            <nav className="admin-pagination" aria-label="Room pages">
              <Button variant="outline" disabled={busy || directory.page <= 1} onClick={() => void load(applied, 1)}>First</Button>
              <Button variant="outline" disabled={busy || directory.page <= 1} onClick={() => void load(applied, directory.page - 1)}>Previous</Button>
              <span>Page {directory.page} of {Math.max(1, Math.ceil(directory.total / directory.pageSize))}</span>
              <Button variant="outline" disabled={busy || directory.page * directory.pageSize >= directory.total} onClick={() => void load(applied, directory.page + 1)}>Next</Button>
            </nav>
          </>}
        </div>
      </section>
      <section className="admin-session-control" aria-labelledby="admin-access-title"><h2 id="admin-access-title">Your access</h2>
        ...
        {!confirmAll ? <Button variant="outline" disabled={busy} onClick={() => setConfirmAll(true)}>Sign out everywhere…</Button> :
          <div className="admin-confirm"><p>End all your administrator sessions now?</p><Button disabled={busy} onClick={() => void sessionAction('logoutAll')}>Confirm sign out everywhere</Button><Button variant="outline" disabled={busy} onClick={() => setConfirmAll(false)}>Cancel</Button></div>}
      </section>
      <p className="admin-secondary admin-development">Administration preview: room creation, lifecycle controls, participant support and backup tools are still being implemented.</p>
    </>}
  </main>;
}
